...
## 📋 **How Authentication Works (CORRECTED)**

### **Step-by-Step Flow:**

**Step 1: Mobile App → Webapp API (Public Call)**

The mobile app talks ONLY to our **production domain** backend (the Next.js API on Vercel).
The mobile app does **NOT** send any secrets.
...
**⚠️ CRITICAL: The mobile app should NOT call `accounting-buddy-app.vercel.app` anymore.**
**It MUST use `https://accounting.siamoon.com` because that is the deployment with the correct environment variables.**

**Step 2: Webapp API → Apps Script (Private Call)**

Inside the **Next.js API route on Vercel** (NOT in the mobile app), we call the Google Apps Script webhook.

**This server code** attaches the secret for authentication:

```javascript
// This code runs ON THE SERVER (Vercel), NOT in the mobile app!
const response = await fetch(process.env.SHEETS_PNL_URL, {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({
    action: 'getPnL',
    secret: process.env.SHEETS_WEBHOOK_SECRET  // This comes from Vercel env vars
  })
});
```

**Important:**
- ✅ This code runs on the server (Vercel)
- ✅ The mobile app never sees `SHEETS_WEBHOOK_SECRET` and never sends it
- ❌ Mobile app does NOT run this code
- ❌ Mobile app does NOT have access to `process.env.SHEETS_WEBHOOK_SECRET`
...
If they don't match, you get "Unauthorized" / 401. ← This is the error you're seeing

**Step 4: Response Flows Back**

If the secret is valid:
- Apps Script queries Google Sheets
- Sends the data back to the Vercel API
- The Vercel API returns JSON to the mobile app

```
Google Sheets → Apps Script → Vercel API → Mobile App
```

---

## 🔎 **Why We're Still Seeing "Unauthorized" Right Now**

### **Two Main Suspects:**

**1. The mobile app is still calling the OLD domain** (accounting-buddy-app.vercel.app), which is using an older deployment / missing env vars.
- **Fix:** Mobile base URL **MUST** be `https://accounting.siamoon.com`

**2. The secret set in Vercel** (`SHEETS_WEBHOOK_SECRET`) does not exactly match `WEBHOOK_SECRET` in Apps Script.
- **Fix:** Copy/paste again and test with curl directly against Apps Script

**These two checks will solve this 99% of the time.**

### **Critical: Secret Must Match EXACTLY**

The secret must be **byte-for-byte identical** in both places:

**In Vercel** (for production domain `accounting.siamoon.com`):
```
...
- Different secret values in preview vs production deployments
...
✅ **DONE!** Mobile app base URL has been updated to `https://accounting.siamoon.com`

Your code is perfect. No further changes needed.

### **What You're Waiting For:**

**PM to ensure Apps Script secret matches Vercel exactly** (5 minutes)

The secret in Apps Script must be:
```
VqwvzpO3Ja5Yn+qhWg6DLwTspv/t2V8f3CXI+iJ9Dz8=
```

No extra spaces, no missing characters, no newlines.
...
|------|--------|-------|------|
| ✅ **Update mobile app base URL** | ✅ **DONE** | **Mobile Team** | **Complete** |
...
**Update:** Mobile app has been updated to use `https://accounting.siamoon.com` ✅

**Current Blocker:** PM needs to update Apps Script secret to match Vercel exactly
